...
<!-- SMS South Africa -->
    <div class="col-lg-6">
        <div class="box p-5 mt-5">
            <div class="d-flex align-items-center justify-content-between p-5 border-bottom border-gray-200 dark-border-dark-5">
            <div class="d-flex align-items-center"><h2 class="fw-medium fs-base me-auto">SMS South Africa</h2> <p><a href="#" data-tooltip="I'm the SMS South Africa"><i class="fa fa-info-circle" style="font-size:24px; margin-top:10px;margin-left:10px;"></i></a></p></div>
    <div class="form-check form-switch w-sm-auto ms-sm-auto mt-3 mt-sm-0 ps-0">
        @php
             $enableSmsSouthAfricaValue = $sms_settings->where('name', 'enable_sms_south_africa')->first(); 
            // Check if $enableSmsSouthAfricaValue is null or 0
            $sms_south_africa_isChecked = ($enableSmsSouthAfricaValue && $enableSmsSouthAfricaValue->value == 1) ? 'checked' : '';
        @endphp        
        <span class="online-status"></span>
        <label class="switch">
            <input type="checkbox" class="online-toggle"  name="enable_sms_south_africa" value="1" {{ $sms_south_africa_isChecked }}>
            <span class="slider round"></span>
        </label>
    </div>
            </div>
            <div class="p-5 text-center"> 
                <img style="margin:auto;" src="{{ asset('assets/img/sms-south-africa.png') }}" width="200px" alt="SMS South Africa" onerror="this.style.display='none'">
                <h4 class="mt-3">SMS South Africa</h4>
            </div>
            <div class="row mt-5">
                <div class="col-12 col-lg-12 mt-5">
                    <label for="" class="form-label">API Key</label> 
                    <input type="text" class="form-control p-3" placeholder=""  name="sms_south_africa_api_key" value="{{ $sms_settings->where('name', 'sms_south_africa_api_key')->first()->value ?? '' }}">                    
                </div>
                <div class="col-12 col-lg-12 mt-5">
                    <label for="" class="form-label">API Secret</label> 
                    <input type="text" class="form-control p-3" placeholder=""  name="sms_south_africa_api_secret" value="{{ $sms_settings->where('name', 'sms_south_africa_api_secret')->first()->value ?? '' }}"> 
                </div>
                <div class="col-12 col-lg-12 mt-5">
                    <label for="" class="form-label">Sender ID</label> 
                    <input type="text" class="form-control p-3" placeholder=""  name="sms_south_africa_sender_id" value="{{ $sms_settings->where('name', 'sms_south_africa_sender_id')->first()->value ?? '' }}"> 
                </div>
            </div>
            <div class="text-end mt-5">
                <button type="submit" class="btn btn-primary w-32">Save</button>
