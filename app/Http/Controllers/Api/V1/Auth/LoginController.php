...
//sms_south-africa    
    //sms south africa
    public function enable_sms_south_africa($mobile, $otp, $country_code)
    {
        $msg = "Dear User, your wait is finally over! Your account OTP is $otp.";

        $apiKey = get_sms_settings('sms_south_africa_api_key');
        $apiSecret = get_sms_settings('sms_south_africa_api_secret');
        $senderId = get_sms_settings('sms_south_africa_sender_id');

        // Remove + from country code if present
        $country_code = str_replace('+', '', $country_code);
        $phoneNumber = $country_code . $mobile;

        // SMS South Africa API endpoint
        $url = 'https://mymobileapi.com/api/v1/sms/send';
        ...
        \Log::info("SMS South Africa Response: " . $response);
        \Log::info("SMS South Africa HTTP Code: " . $httpCode);

        if ($error) {
            \Log::error("SMS South Africa cURL Error: " . $error);
            return response()->json(['success' => false, 'message' => 'SMS sending failed: ' . $error]);
        }

        if ($httpCode !== 200) {
            \Log::error("SMS South Africa HTTP Error: " . $httpCode);
            return response()->json(['success' => false, 'message' => 'SMS sending failed with HTTP code: ' . $httpCode]);
        }
        ...
        if ($responseData && isset($responseData['success']) && $responseData['success']) {
            ...
            \Log::error("SMS South Africa API Error: " . $errorMessage);
            return response()->json(['success' => false, 'message' => 'SMS sending failed: ' . $errorMessage]);
        }
    }
